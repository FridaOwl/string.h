...
- Программы разработаны на языке C стандарта C11 с использованием компилятора gcc в качестве статической библиотеки с заголовочным файлом d3_string.h (см. цель `d3_string.a` в Makefile).
- Код библиотеки, включая заголовочные файлы, Makefile и саму библиотеку, расположен в папке src.
- Написанный код соответствует стилю Google. Устаревшие и устаревшие функции не используются в соответствии с стандартом POSIX.1-2017 (см. цель `style` в Makefile).
- Интеграционные тесты покрывают все функции библиотеки с помощью модульных тестов, используя библиотеку `check.h`. Модульные тесты проверяют результаты реализации, сравнивая их с реализацией стандартной библиотеки string.h (см. цель `test` в Makefile).
- Покрытие модульных тестов проверяется с использованием инструмента gcov. Он предоставляет отчет gcov в виде html-страницы (см. цель `gcov_report` в Makefile).
- Программы разработаны в соответствии с принципами структурированного программирования, дублирование кода избегается.
